...
const signupSSO = async (req,res) =>{
    ...
    if(type=='Customer'){
        ...
        res.status(201).cookie('acessToken',acessToken,options).cookie('refreshToken',refreshToken,options).cookie('refreshToken',refreshToken,options).json({'message':'Customer was created Successfully'})
        
    }else{
        await partnerCreate(req,res);
        const acessToken=await jwtsign(req,res,'1h','a');
        const refreshToken=await jwtsign(req,res,'5d','r');
        res.status(201).cookie('acessToken',acessToken,options).cookie('refreshToken',refreshToken,options).cookie('refreshToken',refreshToken,options).json({'message':'Restaurant was created Successfully'})
    }}catch(err){
